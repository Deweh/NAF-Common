...
namespace Animation::Face
{
	void GraphStub::BeginTween(float a_duration, RE::BSFaceGenAnimationData* a_data)
	{
		for (size_t i = 0; i < FACE_MORPHS_SIZE; i++) {
			snapshot[i] = a_data->finalExpression[i];
		}
	...
	bool GraphStub::Update(float a_deltaTime, RE::BSFaceGenAnimationData* a_data)
	{
		if (transition.state == kNone) {
			for (size_t i = 0; i < FACE_MORPHS_SIZE; i++) {
				a_data->finalExpression[i] = current[i];
			}
			return false;
		}
		...
		if (transition.state == kTween) {
			for (size_t i = 0; i < FACE_MORPHS_SIZE; i++) {
				a_data->finalExpression[i] = std::lerp(snapshot[i], current[i], ratio);
			}
		} else {
			for (size_t i = 0; i < FACE_MORPHS_SIZE; i++) {
				a_data->finalExpression[i] = std::lerp(current[i], a_data->finalExpression[i], ratio);
			}
		}
		return false;
	}
	...
	static Util::Call5Hook<void(RE::BSFaceGenAnimationData*, float)> BlinkUpdateHook(TargetGameVal<size_t>(3139, 113796), TargetGameVal<size_t>(0x53, 0x147), "BSFaceGenAnimationData::UpdateBlinking",
		[](RE::BSFaceGenAnimationData* a1, float a2) {
			...
			if (!m->data.lock_read_only()->noBlink.contains(a1)) {
				...
				//eyeClosed L/R
				a1->finalExpression[25] = 0.0f;
				a1->finalExpression[26] = 0.0f;
#ifdef TARGET_GAME_SF
				a1->expression2[25] = 0.0f;
				a1->expression2[26] = 0.0f;
#endif
			}
		});
	...
	static Util::Call5Hook<bool(RE::BSFaceGenAnimationData*, float, bool, float)> FaceUpdateHook(317245, 0xBA, "BSFaceGenAnimationData::Update",
		[](RE::BSFaceGenAnimationData* a1, float a2, bool a3, float a4) -> bool {
			static Manager* m = Manager::GetSingleton();
			bool res = FaceUpdateHook(a1, a2, a3, a4);
			auto d = m->data.lock_read_only();
			if (auto iter = d->controlledDatas.find(a1); iter != d->controlledDatas.end()) {
				bool requiresDetach = iter->second->lock()->Update(a2, a1);
				if (requiresDetach) {
